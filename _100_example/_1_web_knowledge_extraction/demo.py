...
vectorstore = Chroma(
    embedding_function=OpenAIEmbeddings(model=os.getenv(
...
def main():
    ...
    documents = vectorstore.get()['ids']
    is_store_empty = len(documents) == 0
    ...
    llm = init_chat_model(model=llm_model_name, model_provider="google_genai", temperature=0.5)
